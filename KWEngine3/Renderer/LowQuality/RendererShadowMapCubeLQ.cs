...
{
    internal class RendererShadowMapCubeLQ : IRenderer
    {
        ...
        public void Init()
        {
            if (ProgramID < 0)
            {
                ...
                string resourceNameVertexShader = "KWEngine3.Shaders.LowQuality.shader.msm16.cubeLQ.vert";
                string resourceNameGeometryShader = "KWEngine3.Shaders.LowQuality.shader.msm16.cubeLQ.geom";
                string resourceNameFragmentShader = "KWEngine3.Shaders.LowQuality.shader.msm16.cubeLQ.frag";
            ...
            foreach (GameObject g in KWEngine.CurrentWorld.GetGameObjects())
            {
                if(g.IsShadowCaster && g._stateRender._opacity > 0 && g.IsAffectedByLight)
                {
                    if (l._frustumShadowMap.IsBoxInFrustum(
                            ...
                            g.Center,
                            new Vector3(g.AABBLeft, g.AABBLow, g.AABBBack),
                            new Vector3(g.AABBRight, g.AABBHigh, g.AABBFront),
                            g._stateRender._dimensions.LengthFast))
                        Draw(g);
                }
            }
