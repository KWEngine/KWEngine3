            ...
            if (cubeMap)
            {
                for (int j = 0; j < 6; j++)
                {
                    GL.TexImage2D(
                        ...
                        height, 
                        0,
                        mode == FramebufferTextureMode.DEPTH32F || mode == FramebufferTextureMode.DEPTH16F ? PixelFormat.DepthComponent : (mode == FramebufferTextureMode.RGB8 || mode == FramebufferTextureMode.RGB16F )? PixelFormat.Rgb : (mode == FramebufferTextureMode.RG8 || mode == FramebufferTextureMode.RG32I) ? PixelFormat.Rg : mode == FramebufferTextureMode.R32F ?  PixelFormat.Red : PixelFormat.Rgba,
                        mode == FramebufferTextureMode.RGBA16UI ? PixelType.UnsignedShort : (mode == FramebufferTextureMode.RGBA8 || mode == FramebufferTextureMode.RG8) ? PixelType.UnsignedByte : (mode == FramebufferTextureMode.RGBA16F || mode == FramebufferTextureMode.RGB16F) ? PixelType.HalfFloat : mode == FramebufferTextureMode.RG32I ? PixelType.Int : PixelType.Float,
                        IntPtr.Zero);
                }
