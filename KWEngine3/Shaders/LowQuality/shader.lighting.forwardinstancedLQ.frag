...
float calculateShadowCube(int index, vec3 lightPos, vec3 fragPos, vec2 lightNearFar, float bias, float hardness)
{
    ...
    sampledDepthMSM.r = sampledDepthMSM.r * 2.0 - 1.0;
    sampledDepthMSM.r = lightNearFar.x * lightNearFar.y / (lightNearFar.y + sampledDepthMSM.r * (lightNearFar.x - lightNearFar.y));
