...
namespace KWEngine3TestProject.Classes.WorldFollowerCam
{
    internal class Camera : GameObject
    {
        ...
        private const float MOUSE_MOVEMENT_MINUMUM = 0.000001f;    // Menge an Mausbewegung, ab die der Orb die Maus als "sich bewegend" sieht
        ...
        private const float FOLLOW_DISTANCE = 4f;              // Standarddistanz zum Player
        private const float FOLLOW_OFFSET_Y_NEAR = 1f;          // Y-Abstand zur Player-Höhe (für den Messpunkt direkt am Player)

        private float _followStrength = 1f;                    // Je höher die Zahl, desto schneller passt sich die Kamera (bzw. der Orb) an
        ...
        public void UpdateViewFor(Player _parent)
        {
            ...
            Vector3 distanceToPlayer = HelperVector.GetDeltaBetweenVectors(newPosition, pivot);
            ...
            float tRotation = 1f - MathF.Exp(-_followStrength * 20f * SIMULATION_FRAME_TIME);
            ...
            else
            {
                ...
                if (pMotion != Vector3.Zero)
                {
                    ...
                    float dotLeftRight = Vector3.Dot(CurrentWorld.CameraLookAtVectorLocalRightXZ, pMotionNormalized);

                    if (Math.Abs(dotLeftRight) > 0.5f)
                    {
                        Vector3 strafePosition = Position + pMotion;
                        newPosition = strafePosition;
                    ...
                    else 
                    {
                        ...
                        {
                            newPosition = pivot - directionToPlayerXZ * _distance + new Vector3(0f, 0.25f * (pivot - directionToPlayer).Y, 0f);
                        }
